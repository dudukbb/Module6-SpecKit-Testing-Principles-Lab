...
describe("task routes integration", () => {
  ...
  it("should create task and include warning when service returns one", async () => {
    ...
    it("should list tasks with empty query param and normalize to undefined", async () => {
      // Arrange
      mockedListUserTasks.mockResolvedValueOnce([]);
      ...
      // Assert
      expect(response.status).toBe(200);
      expect(mockedListUserTasks).toHaveBeenCalledWith("u-auth", expect.objectContaining({ status: [""] }));
    });
